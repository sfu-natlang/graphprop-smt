...
mkdir -p $output_path/computeInvIndx
hadoop fs -getmerge "$hdfs_output/part*" $output_path/computeInvIndx/$input_file_name.$expr_date.indx
#hadoop fs -rm $path_hdfs/$input_file_name

echo "Final output is in :$output_path/computeInvIndx/$input_file_name.$expr_date.indx"

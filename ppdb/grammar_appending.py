                        ...
                        if phrase in self.oov_list:
                            ...
                            if self.extractor.contains(phrase):
                                ...
                                for (rule,p) in label_list:
                                    prob = float(p)
                                    lexf = str(float(len(phrase)))
                                    lexe = str(float(len(rule.split(" ||| ")[2])))
                                    srule = rule.replace('~~~',phrase, 1)+" ||| EgivenFCoherent=2.47712135315 SampleCountF=2.47856640816 CountEF=1.0 MaxLexFgivenE="+lexf+" MaxLexEgivenF="+lexe+" IsSingletonF=0.0 IsSingletonFE=0.0 newF=1.0 "+"Seed="+ str(prob)


                                    srule = rule.replace('~~~',phrase, 1)+" ||| EgivenFCoherent=1.0 SampleCountF=1.0 CountEF=1.0 MaxLexFgivenE=1.0 MaxLexEgivenF=1.0 IsSingletonF=1.0 IsSingletonFE=1.0 newF=1.0 "+"Seed="+ str(prob)
                    ...
                    for word in words:
                        ...
                        if word in self.oov_list:
                            ...
                            if self.extractor.contains(word):
                                ...
                                for (rule,p) in label_list:
                                    prob = float(p)
                                    lexf = str(float(len(word)))
                                    lexe = str(float(len(rule.split(" ||| ")[2])))
                                    srule = rule.replace('~~~',word, 1)+" ||| EgivenFCoherent=2.47712135315 SampleCountF=2.47856640816 CountEF=1.0 MaxLexFgivenE="+lexf+" MaxLexEgivenF=1.0 IsSingletonF=1.0 IsSingletonFE=1.0 newF=1.0 "+"Seed="+ str(prob)
                                    new_grammar_file.write(srule+"\n")
                                    #print srule + "\n"
                grammar_file.close()
                new_grammar_file.close()
...
if __name__=="__main__":
    ...
    testExtractor.extract(phrase_list_file=MT_prefix+"/dev.phrases", propagation_output=propagation_prefix+"_finalitr1", id_to_phrase_file=propagation_prefix+"_id_to_phrase.pkl",id_to_label_file=propagation_prefix+"_id_to_label.pkl", max_number_of_labels=100)
#      
    testGrammarGenerator = grammarGenerator(extractor=testExtractor, grammar_files_path=MT_prefix+"/old/dev.grammars", grammar_output_files_path=MT_prefix+"/europarl_en-es_m_phrasal_itr1_newfeatures/dev.grammars", text_file=MT_prefix+"/dev.en-es", oov_list_file=MT_prefix+"/dev.phrases")
#
    testGrammarGenerator.unseen_phrases(GSCFG =True)
#
    testExtractor.extract(phrase_list_file=MT_prefix+"/devtest.phrases", propagation_output=propagation_prefix+"_finalitr1", id_to_phrase_file=propagation_prefix+"_id_to_phrase.pkl",id_to_label_file=propagation_prefix+"_id_to_label.pkl", max_number_of_labels=100)
#
#
    testGrammarGenerator = grammarGenerator(extractor=testExtractor, grammar_files_path=MT_prefix+"/old/devtest.grammars", grammar_output_files_path=MT_prefix+"/europarl_en-es_m_phrasal_itr1_newfeatures/devtest.grammars", text_file=MT_prefix+"/devtest.en-es", oov_list_file=MT_prefix+"/devtest.phrases")
#
    testGrammarGenerator.unseen_phrases(GSCFG =True)

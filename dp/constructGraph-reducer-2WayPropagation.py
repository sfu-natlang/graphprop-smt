...
def load_phrase_table():
    ...
    for line in f:
        ...
        phrase_table[src].append((tgt, float(probs.split()[2]), float(probs.split()[0])))
...
def reducer():
    ...
    #for line in open('old/DPs.out'):
        ...
        if phr in phrase_table:
            ...
            for word, prob, rev_prob in translations[:max_translations]:
                print '***%s\t%s\t%f'%(phr, word, prob)
                print '###%s\t%s\t%f'%(phr, word, rev_prob)

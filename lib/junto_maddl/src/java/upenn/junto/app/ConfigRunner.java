  ...
  public static Graph Run(Hashtable config, ArrayList resultList) {
    ...
    if (algo.equals("adsorption") || algo.equals("mad")) {
      ...
      LP_ZGL.Run(g,
                 ...
                 verbose,
                 resultList);
    } else if (algo.equals("maddl")) {
		config.put("mode", "modified");
		double mu4 = Defaults.GetValueOrDefault((String) config.get("mu4"), 1.0);
		
		// load label graph
		LabelGraph lc = new LabelGraph();
		lc.LoadLabelGraph(g._labelAlphabet, // g._labels,
					      (String) config.get("label_graph_file"));
